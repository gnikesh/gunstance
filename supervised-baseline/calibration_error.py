...
def get_best_temperature(elems, n_classes):
    ...
    best_temp = None
    for temp in temp_values: #tqdm(temp_values, leave=False, desc='finding best temperature'):
        nll = np.mean(
            [
                cross_entropy(
...
def calculate_calibration_error(elems, n_classes, temperature=None, average='weighted'):
    ...
    pr, rc, f1, sp = precision_recall_fscore_support(labels, preds, average=average) # type: ignore
